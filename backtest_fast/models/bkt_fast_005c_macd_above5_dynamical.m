classdef bkt_fast_005c_macd_above5_dynamical < handle
    ...
    methods
        ...
        function obj = spin(obj, Pminute, matrixNewHisData, ~, newTimeScale, N, ~, cost, ~, ~, wTP, wSL, plottami)
            ...
            % se il vettore macdvec oscilla sopra il 5.
            % Il TP e SL sono settati da quando parte il primo segnale
            ...
            stempup = sup + myvec;
            stempdown = sdown - myvec;
            
            stempup(stempup<2)=0;
            stempdown(stempdown>-2)=0;
            
            s = (stempup + stempdown)/2;
            ...
            while i < sizeStorico
                ...
                if  ( abs( s(i) - s(i-1) ) == 2 )
                    ...
                    % prendo come range di SL e TP meta' del balzo che ha
                    % fatto il prezzo prima di cambiar segnale
                    for iback = (i-2):-1:(i-100)
                       
                        if (s(iback) == -segnoOperazione)
                            myprice = P(iback);
                        else
                            break
                        end
                    ...
                    TakeP = min(floor(abs(Pbuy-myprice)/2),50);
                    StopL = TakeP;
