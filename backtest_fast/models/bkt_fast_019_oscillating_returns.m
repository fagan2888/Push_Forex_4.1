classdef bkt_fast_019_oscillating_returns < handle
    ...
    methods
        ...
        function obj = spin(obj, Pminute, matrixNewHisData, ~, newTimeScale, jump_pips, ~, cost, ~, ~, ~, ~, plottami)
            ...
            returns_pips = [0; P(2:end)-P(1:end-1)];
            
            s( returns_pips > jump_pips ) = 1;
            s( returns_pips < -jump_pips ) = -1;
            
            i = 101;
            
            
            while i < sizeStorico-2 % occchio che qui ho aggiunto -2 xe sposto i di due se scatta il trigger
                
                % se c'e' stato un jump_pips nei ritorni, aspetta che torni
                % a zero o sotto (almeno 2 periodi) e apri nel senso del jump
                if   ( abs(s(i)) == 1 )
                    
                    segnoOperazione = -s(i); % stesso segno del jump
                    
                    i=i+2; % aspetta due turni da quando c'e' stato il jump
                    
                    if ( returns_pips(i)*(-segnoOperazione) > 2 )
                        i=i+1;
                        continue
                    end
