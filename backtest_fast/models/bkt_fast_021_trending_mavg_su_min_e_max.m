classdef bkt_fast_021_trending_mavg_su_min_e_max < handle
    ...
    methods
        ...
        function obj = spin(obj, Pminute, matrixNewHisData, ~, newTimeScale, N, M, cost, ~, ~, wTP, wSL, plottami)
            ...
            % N = lookback period per vedere se sei su un max o min
            % jump = minimo delta di pips necessario tra max e min del periodo x dar il segnale
            ...
            ntrades = 0;
            obj.indexClose = 0;
            sMax = zeros(sizeStorico,1);
            sMin = zeros(sizeStorico,1);
            
            a = (1/N)*ones(1,N);
            smoothedMax = filter(a,1,hi);
            smoothedMin = filter(a,1,lo);
            
            diffMax = [0 ; diff(smoothedMax)];
            diffMin= [0 ; diff(smoothedMin)];
            ...
            while i < sizeStorico
                ...
                % se le derivate sono concordi, apri
                if   ( abs(sMax(i)) == 1 && sMax(i)==sMin(i) && (P(i-1)-op(i-1))==0 &&  sMax(i)*(P(i)-op(i))> 5 && sMax(i)*(P(i)-P(i-1))< 50 )
                    
                    segnoOperazione = sMax(i);
