...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
      ...
      string strline2 = DoubleToString(iOpen  (Symbol(), PERIOD_M1, 0), 5);
      string strline3 = DoubleToString(iHigh  (Symbol(), PERIOD_M1, 0), 5);
      string strline4 = DoubleToString(iLow   (Symbol(), PERIOD_M1, 0), 5);
      string strline  = DoubleToString(iClose (Symbol(), PERIOD_M1, 0), 5);
      string strline5 = DoubleToString(iVolume(Symbol(), PERIOD_M1, 0), 5);
      string strline6 = DoubleToString(Bid(), 5);
            int day = Day();
            string s_day;
            if (day < 10) {
               s_day = "0" + day;
      ...
      string strline7 = TimeToString(TimeCurrent(),TIME_MINUTES);

      double strline_intero=StringToDouble(strline);
      double strline_intero2=StringToDouble(strline2);
      double strline_intero3=StringToDouble(strline3);
      double strline_intero4=StringToDouble(strline4);
      double strline_intero5=StringToDouble(strline5);
      double strline_intero6=StringToDouble(strline6);
...
void write_history(int period) {    
      ...
      for (cnt = number_bars; cnt >= shift; cnt--) {
         ...
         if (from_year < TimeYear(iTime(Symbol(), period, cnt))) {
            ...
            string strline2 = DoubleToString(iOpen  (Symbol(), period, cnt), 4);
            string strline3 = DoubleToString(iHigh  (Symbol(), period, cnt), 4);
            string strline4 = DoubleToString(iLow   (Symbol(), period, cnt), 4);
            string strline  = DoubleToString(iClose (Symbol(), period, cnt), 4);
            string strline5 = DoubleToString(iVolume(Symbol(), period, cnt), 4);
            string strline6 = TimeToString(iTime  (Symbol(), period, cnt),TIME_MINUTES);
...
int sleepXIndicators(int milli_seconds)
  ...
  void OnTimer() {
   ...
   bool isOpen = market_open(Symbol());
   string isMarketOpen;
   if(isOpen) isMarketOpen = "1";
   else isMarketOpen = "0";
