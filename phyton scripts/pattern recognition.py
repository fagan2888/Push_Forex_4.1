# -*- coding: utf-8 -*-
"""
Created on Sun Jan  8 11:35:34 2017
...
def patternStorage():
    ...
    # Si salva ogni pattern possibile di lunghezza 30 che vede sullo storico 
    # in termini di variazione percentuale tra un punto e quello sucessivo
    # e l'array del ritorno medio futuro
    ...
    print('length of pattern array: ', len(patternArr) )
    print('length of performance array (the same): ', len(performanceArr) )
    print('pattern storage took: ', patEndTime-patStartTime, ' seconds')
        

def currentPattern():
    
    # si calcola i ritorni del pattern corrente
...
def patternRecognition():
    ...
    # per ogni pattern for recognition guarda la somiglianza con tutti gli altri
    # pattern salvati (eachPattern di patternArr) e calcola i ritorni attesi medi e quello reale
    ...
    for eachPattern in patternArr:
        ...
        howSim = (sim1 + sim2 + sim3 + sim4 + sim5 + sim6 + sim7 + sim8 + sim9 + sim10 +
                  sim11 + sim12 + sim13 + sim14 + sim15 + sim16 + sim17 + sim18 + sim19 +
                  sim20 + sim21 + sim22 + sim23 + sim24 + sim25 + sim26 + sim27 + sim28 +
                  sim29 + sim30 )/30.0
...
while toWhat < dataLength:
    ...
    samps += 1
    toWhat += 1
    if len(accuracyArr) > 0:
        accuracyAvg = reduce(lambda x, y: x+y, accuracyArr) / len(accuracyArr)
        print('BKT accuracy: ', str(accuracyAvg)+'% after ',samps,' samples')
