function [outputWP]=fromWebOperToMatrix_03(AlgoMagicNumber,dateFirstNum,dateLastNum,newTimeScale,FullnameOrServername)
...
for i = 1: l
    magic = index(i);
    dateOpen  = indexDateOpen(i);
    dateClose = indexDateclose(i);
    if (magic == AlgoMagicNumber) 
        if (dateOpen >= dateFirstNum) && (dateClose <= dateLastNum)
        row(i,1)=1;
        end
    else
        row(i,1)=0;
